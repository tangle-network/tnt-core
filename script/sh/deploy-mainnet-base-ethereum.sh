...
# Deploy orchestrator for an Ethereum mainnet (L1) ↔ Base mainnet (L2) environment.
# - Base: deploy protocol core (FullDeploy) + L2 slashing receiver
# - Ethereum: deploy beacon slashing connector + messenger, then wire to Base receiver
#
# Prereqs:
# - jq installed
...
L1_MANIFEST_PATH="${L1_MANIFEST_PATH:-deployments/base-mainnet/beacon-slashing.json}"
L2_SLASHING_MANIFEST_PATH="${L2_SLASHING_MANIFEST_PATH:-deployments/base-mainnet/l2-slashing.json}"

echo "==> 1/4 Deploy protocol core on Base mainnet"
...
echo "==> 4/4 Wire Ethereum mainnet connector -> Base mainnet receiver"
