...
\begin{abstract}
Tangle Network is a decentralized protocol for coordinating computational services with cryptoeconomic accountability. This document describes the protocol's architecture, economic mechanisms, and security model. Developers define service templates (blueprints), operators provide compute with staked collateral, and customers consume services with cryptographic guarantees. The core accounting paths for staking, slashing, and reward distribution are O(1), so gas cost does not grow with participant count. Operators stake assets that can be destroyed if they misbehave, which makes honest service the economically rational path. The network token (TNT) is hard-capped at 100{,}000{,}000 TNT,
...
Increasingly capable AI systems need infrastructure that can scale without losing accountability. Current options fall into two weak categories: centralized cloud providers that can revoke access or change terms, and decentralized compute markets that often lack the coordination needed for production workloads. Centralized systems also concentrate pricing power, operational control, and customer relationships in a few companies.

Tangle fills that gap with a protocol layer for computational services. Developers define service templates. Operators provide compute and stake collateral. Customers request service instances and pay for work. Service fees and treasury-funded emission then flow to participants according to contribution rather than control of distribution.
...
Traditional cloud infrastructure provides compute but not hard accountability. Terms can change, access can be revoked, and the provider set is small. Dependence becomes market power. For infrastructure that controls meaningful economic activity, that is a real risk.
...
The dominant model for AI infrastructure concentrates value in cloud companies. They capture margin on every API call and own the customer relationship. Developers and operators become interchangeable suppliers.
...
The inflation distribution allocates 10\% to developers and 25\% to operators. Blueprint creators share in network growth alongside compute providers. Payment splits route value to participants who earned it. Governance grants voting power proportional to stake.
...
The core accounting paths are O(1). Share-based accounting tracks delegation through exchange rates rather than individual balances. Accumulated-per-share rewards track distributions through single accumulators. Proportional slashing adjusts exchange rates without iterating over positions.

This is a design requirement, not a micro-optimization. A protocol that iterates over every delegator cannot support a large operator market on an EVM chain.
...
The blueprint system is Tangle's service architecture. A blueprint defines a service type. A service instance is the live deployment of that blueprint with a selected operator set.

\subsection{What Is a Blueprint?}

A blueprint is a reusable template for a service type. Developers use it to specify computational tasks, pricing models, operator requirements, and optional custom logic through hooks.
...
A service instance is a running deployment of a blueprint with assigned operators. The blueprint says what can be run; the service instance is the thing customers actually use.
...
\textbf{Dynamic membership} permits operators to join after activation and leave subject to exit queue constraints. Long-running services can change operator sets without recreating the blueprint.
...
MBSM handles a practical upgrade problem: the protocol needs to evolve, but live services need stable rules. A trading-agent service instance cannot have its service manager change mid-operation. Pinning gives that service a fixed manager while the protocol can still ship new MBSM versions.
...
Request-for-quote asks operators to provide binding prices for specific requests. Customers compare price, resource mix, exposure, and operator history, then select a quote. RFQ fits heterogeneous operators, requires no standing on-chain order book, and gives final pricing at service creation.
...
Upgrades are the most sensitive governance action. A malicious upgrade could drain funds or disable slashing. Every upgrade proposal should be treated as dangerous until proven otherwise.
...
Tangle prevents this through temporal separation. Stake and delegations cannot be acquired and withdrawn within a single transaction: unbonding clears only after multi-day, round-based exit delays (delegator 28 rounds, $\sim$7 days; operator 56 rounds, $\sim$14 days), so a flash loan repaid in the same transaction can never both stake and exit. Slashing proposals also survive their dispute window and the 21-day fail-open resolution deadline, long after any flash loan repayment.
...
\textbf{Blueprint abstraction.} EigenLayer requires deploying new AVS contracts for each service type. Tangle defines service types through blueprints: same protocol contracts, different parameters and hooks. A developer creates a price oracle, an AI inference service, and a keeper bot using identical tooling.

\textbf{Granular exposure control.} Most restaking protocols offer binary participation: opt in or out per service. Tangle's exposure model lets operators tune risk, for example by joining a higher-risk service with 20\% exposure while keeping the rest of their stake available. Operators do not have to make all-or-nothing risk decisions.
...
\textbf{Developer economics.} Tangle allocates 10\% of treasury-funded emission to blueprint developers alongside 25\% to operators. Service creators get a direct reward path instead of relying only on optional fees.
...
Tangle provides the coordination layer for a decentralized compute market. Developers define computational services. Operators provide those services with economic accountability. Customers consume them with cryptographic guarantees.

The mechanisms described here, blueprints and services, staking and delegation, RFQ pricing, fee distribution, slashing, and governance, are designed around one rule: honest participation should be more profitable than cheating. Decentralization is a launch trajectory, not a launch-day fact. At mainnet the UUPS upgrade key sits behind the 4-day governance timelock and a guardian multisig, and most supply is still vesting-locked. As vesting unlocks deepen the delegatable float and governance quorum ratchets up, control diffuses toward operators, developers, and delegators.

The practical argument is straightforward. Decentralized infrastructure reduces provider concentration. Competitive markets pressure pricing. Cryptographic verification gives customers evidence rather than promises. The same properties that made decentralized finance useful apply to decentralized AI infrastructure.

Autonomous work needs an accountability layer. Tangle is built to be that layer.
