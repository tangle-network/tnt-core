...
Treasury & $\sim$41.84M \\ %% PENDING-100M: if normalized to 100M total, treasury ~32.6M and emission ~1,000,000 TNT/yr
...
The InflationPool uses a pre-funded model. The pool cannot mint tokens; it distributes tokens it holds. Emission is funded by transfers from the Treasury bucket into the pool via \texttt{fund()}, targeting roughly 1\% of supply per year ($\sim$1{,}092{,}556 TNT/yr). %% PENDING-100M: if normalized to 100M total, treasury ~32.6M and emission ~1,000,000 TNT/yr
