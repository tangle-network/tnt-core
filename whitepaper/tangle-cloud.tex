...
TNT is hard-capped: \texttt{MAX\_SUPPLY} = 100,000,000 TNT.
Genesis mints the full cap across four buckets (Substrate $\approx$51.24M, EVM $\approx$1.13M, Treasury $\approx$32.59M, Foundation $\approx$15.04M), so \texttt{totalSupply} equals the cap at genesis.
The protocol is hard-capped; the InflationPool cannot mint; emission is treasury-funded; only governance can mint and only up to the cap (no headroom at genesis). ``Emission'' is treasury-funded: $\approx$1\% of supply per year ($\approx$1,000,000 TNT)
