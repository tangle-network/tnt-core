...
\begin{description}
    ...
    \item[Developers] create blueprints defining service behavior and earn from fee splits and emission rewards proportional to adoption.
    \item[Delegators] provide additional stake to operators and share in operator rewards proportional to their delegation.
    \item[Protocol] captures a governance-controlled fee (currently 19.5\%, with a 0.5\% keeper rebate carved out so permissionless billing is EV-positive) funding development, security audits, and ecosystem growth.
\end{description}

Service payments split across five recipients in the service's payment asset, as summarized in Table~\ref{tab:feesplit}.

\begin{table}[h]
\centering
\caption{Canonical service fee split (sums to 100\%).}
\label{tab:feesplit}
\begin{tabular}{lr}
\toprule
...
TNT is hard-capped: \texttt{MAX\_SUPPLY} = 109,255,636.92 TNT. %% PENDING-100M: if normalized to 100M total, treasury ~32.6M and emission ~1,000,000 TNT/yr
Genesis mints the full cap across four buckets (Substrate $\approx$51.24M, EVM $\approx$1.13M, Treasury $\approx$41.84M, Foundation $\approx$15.04M), so \texttt{totalSupply} equals the cap at genesis. %% PENDING-100M: if normalized to 100M total, treasury ~32.6M and emission ~1,000,000 TNT/yr
The protocol is hard-capped; the InflationPool cannot mint; emission is treasury-funded; only governance can mint and only up to the cap (no headroom at genesis). ``Emission'' is treasury-funded: $\approx$1\% of supply per year ($\approx$1,092,556 TNT) %% PENDING-100M: if normalized to 100M total, treasury ~32.6M and emission ~1,000,000 TNT/yr
is transferred from the Treasury bucket into the InflationPool; it dilutes no one and creates no new supply.

Migrated balances are claimed via an SP1 Groth16 zk-proof of Substrate key ownership plus a merkle proof. Vesting unlocks 10\% at claim, then a 12-month cliff and 24-month linear release; treasury and foundation allocations vest over $\approx$3 years (foundation with 30\% unlocked at start). See the \textit{Tangle Protocol Specification} for the full supply and distribution schedule.
...
\begin{enumerate}[noitemsep]
    ...
    \item Operators return EIP-712 signed quotes with pricing
    \item Customer accepts selected quotes on-chain, where signatures are verified
    \item Service activates at the committed prices
\end{enumerate}
...
\begin{enumerate}[noitemsep]
    ...
    \item Protocol handles payment splits (see Table~\ref{tab:feesplit})
\end{enumerate}
...
\textbf{Accountable AI for high-stakes decisions}: A trading firm needs confidence that analysis used the specified model and data. Operators are held accountable through stake and slashing; cryptographic verification of model and data integrity (ZK/TEE) is on the roadmap.
...
\textbf{Developer monetization at scale}: Create a blueprint; earn from every service instantiation across all operators. Treasury-funded emission provides additional rewards proportional to adoption.
...
\textbf{Emission rewards}: A fixed 25\% operator share of treasury-funded emission, distributed by stake weight and active-service participation. The exposure rail starts at 0 until live services and the fee distributor exist.
...
Treasury-funded emission is distributed across five buckets, summarized in Table~\ref{tab:emission}. The stakers (exposure) bucket is the exposure rail and starts at 0\% at genesis because live services and the fee distributor are not yet active (beacon-chain restaking and cross-L2 slashing are Phase 2; see Section~6).

\begin{table}[h]
\centering
\caption{Emission allocation and distribution weights (sums to 100\%).}
\label{tab:emission}
\begin{tabular}{lr}
\toprule
\textbf{Bucket} & \textbf{Weight} \\
\midrule
Staking & 55\% \\
Operators & 25\% \\
Customers & 10\% \\
...
\textbf{Subscription}: Predictable costs for recurring workloads. Customers fund escrow; anyone can permissionlessly trigger TWAP-fair billing at intervals, earning the 0.5\% keeper rebate.
...
\textbf{RFQ quotes}: Operator-signed EIP-712 quotes that the customer accepts on-chain at the committed price.
...
\begin{description}
    \item[Staking] is required for operator participation. Operators self-bond at least 1,000 TNT (the bond is always TNT; delegation may accept other assets, but the operator bond does not). The amount staked determines service capacity and enables operators to accept higher-value workloads requiring greater economic security.
    \item[Delegation] allows passive holders to earn yield by backing operators. The minimum delegation is 0.2 TNT. Delegators share in operator rewards (operator commission defaults to 10\%, with a 7-day timelock on changes) proportional to their contribution. Longer lock commitments earn multipliers up to 1.6x (timestamp-based: 1.0/1.1/1.2/1.3/1.6x at 0/1/2/3/6 months). Unbonding takes $\approx$7 days for delegators and $\approx$14 days for operators. A liquid delegation vault (ERC-7540) is also available, with a 15\% default operator commission.
    \item[Governance] grants voting rights on protocol parameters. Token holders can propose and vote on fee structures, emission allocation and distribution weights, and protocol upgrades.
    \item[Payment] is made in the service's chosen payment asset; staking, operator bonds, and governance are denominated in TNT. Fee discounts for paying in TNT are on the roadmap.
    \item[Fee distribution] flows in the payment asset to recipients, creating coherent value circulation where network activity generates rewards that flow back to participants.
\end{description}
...
\textbf{Core protocol contracts}: Feature-complete and undergoing external security audit ahead of mainnet on Base. The full service lifecycle is implemented including: blueprint registration, operator staking, service creation via RFQ, job execution, payment distribution, and slashing with dispute windows. The contracts are a custom facet router behind a UUPS proxy (the fallback resolves a selector to one of the core mixin facets and delegatecalls into it over shared proxy storage); this is not an EIP-2535 Diamond (no \texttt{diamondCut}/loupe), and upgrades go through the UUPS path under governance. See \textit{Tangle Protocol Specification} for mechanism details.
...
\begin{enumerate}[noitemsep]
    \item Parameter governance (fee rates, emission allocation and distribution weights)
...
\textbf{Cross-chain deployment}: Tangle Cloud launches on Base; cross-L2 slashing (OP-Stack native messenger) is Phase 2.
\begin{itemize}[noitemsep]
    \item Additional L2s: Arbitrum, Optimism for lower transaction costs
...
\begin{description}
    ...
    \item[Fee volume] measures total service-payment volume, of which the protocol takes a 19.5\% slice, indicating real economic activity.
    \item[Stake growth] tracks total value locked, reflecting increasing security capacity for higher-value workloads.
\end{description}

These metrics guide parameter adjustments (emission allocation and distribution weights, fee structures, minimum stakes) to ensure sustainable growth.
...
\textbf{Token volatility}: Slashing effectiveness depends on stake value. Rapid token price declines can reduce security margins below safe thresholds faster than services can respond. Bounded slashing (the 50\% per-service cap), the dispute window, and the pending-slash unstake freeze constrain the damage but cannot eliminate this risk.

\textbf{Cold start problem}: Two-sided marketplaces face bootstrapping challenges. Customers need operators; operators need customers. Early network growth is funded by treasury-funded emission, a finite runway ($\approx$1\% of supply per year) that must transition to sustainable fee-based economics before the treasury bucket is exhausted. %% PENDING-100M: if normalized to 100M total, treasury ~32.6M and emission ~1,000,000 TNT/yr

\textbf{Thin delegatable float and quorum}: At launch most supply is vesting-locked, so the governance quorum is set at only 4\% of total supply. A canceller guardian multisig and the 4-day timelock are the primary capture defense until vesting unlocks and quorum ratchets up.
