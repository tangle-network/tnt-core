...
\textbf{BuybackBlueprintBase} adds AMM buyback mechanics. A portion of service fees is automatically used to buy back the blueprint's community token on decentralized exchanges, creating sustained demand. Blueprint developers configure the buyback percentage and target AMM pool. The buyback is funded from the blueprint developer's share of the service fee split (see Payment Models); it does not draw from operator, staker, or protocol shares.

These extensions compose with each other and with custom hooks, enabling blueprints to build token economies around their services. They are opt-in base contracts: a blueprint must explicitly extend and deploy them, and they are not auto-deployed by the protocol. Treat them as early-maturity primitives. On-chain buybacks execute against public AMM pools and are therefore MEV-exposed (front-running, sandwiching); developers should bound slippage, consider TWAP execution, and size buybacks conservatively relative to pool depth.
...
TNT is hard-capped at \texttt{MAX\_SUPPLY} $=$ 109{,}255{,}636.92 TNT, %% PENDING-100M: if normalized to 100M total, treasury ~32.6M and emission ~1,000,000 TNT/yr
and the full cap is minted at genesis across four buckets (Substrate, EVM, Treasury, Foundation), so \texttt{totalSupply} equals the cap from block zero. The InflationPool cannot mint; it only distributes TNT it already holds. ``Inflation'' here is treasury-funded \emph{emission}: roughly 1\% of supply per year ($\approx$ 1{,}092{,}556 TNT) %% PENDING-100M: if normalized to 100M total, treasury ~32.6M and emission ~1,000,000 TNT/yr
