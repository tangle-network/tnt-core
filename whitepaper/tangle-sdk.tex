...
% Rust code styling (listings has no native Rust language; keywords supplied explicitly)
\lstdefinestyle{rust}{
...
% Solidity code styling (listings has no native Solidity language; keywords supplied explicitly)
\lstdefinestyle{solidity}{
    ...
    frame=single,
    backgroundcolor=\color{gray!5},
    morekeywords={contract,function,external,public,internal,private,view,pure,payable,override,returns,mapping,address,uint8,uint16,uint64,uint256,bytes,bytes32,string,memory,calldata,storage,require,emit,struct,constant,if,else,return},
    morestring=[b]",
    morecomment=[l]{//},
    morecomment=[s]{/*}{*/},
}
...
\begin{itemize}
    ...
    \item \textbf{WebAssembly compilation}: Lets SDK code run in sandboxed environments.
    \item \textbf{Tooling compatibility}: Strong integration with blockchain tooling (alloy, libp2p, ark-bn254).
\end{itemize}
...
\begin{lstlisting}
[dependencies]
blueprint-sdk = { git = "https://github.com/tangle-network/blueprint", branch = "main", features = ["tangle"] }
...
\begin{description}
    ...
    \item[onUnappliedSlash] executes custom slashing logic when a slash is finalized. A threshold service might redistribute key shares. The \texttt{slashPercent} argument is the dispute-survived, per-service-capped slash as a whole percent (0--100); note that \texttt{proposeSlash} takes the proposed amount in basis points while BSM callbacks such as this one receive a whole percent (a deliberately different unit, not a bug).
...
\textbf{TokenizedBlueprintBase} lets blueprints issue community tokens tied to their service. Operators and users can stake these tokens to earn Synthetix-style staking rewards, so adoption can reward the participants backing the blueprint. The base contract handles token issuance, staking accounting, and reward distribution.

\textbf{BuybackBlueprintBase} adds AMM buyback mechanics. A portion of service fees is automatically used to buy back the blueprint's community token on decentralized exchanges, creating recurring buy pressure. Blueprint developers configure the buyback percentage and target AMM pool. The buyback is funded from the blueprint developer's share of the service fee split (see Payment Models); it does not draw from operator, staker, or protocol shares.

These extensions compose with each other and with custom hooks. They are opt-in base contracts: a blueprint must explicitly extend and deploy them, and they are not auto-deployed by the protocol. Treat them as early-maturity primitives. On-chain buybacks execute against public AMM pools and are therefore MEV-exposed (front-running, sandwiching); developers should bound slippage, consider TWAP execution, and size buybacks conservatively relative to pool depth.

\subsection{Slashing Customization}

Blueprints define their own slashing conditions:

\texttt{querySlashingOrigin} returns a \emph{single} authorized address that may propose slashes (in addition to the service owner and blueprint owner). There is no built-in permissionless ``anyone with evidence'' path: to support open, evidence-based proposals, point \texttt{querySlashingOrigin} at a verification contract you deploy that validates submitted evidence and forwards \texttt{proposeSlash} on the caller's behalf.
...
Membership is configured on the service request, not through blueprint hooks. A request specifies a \texttt{Types.MembershipModel} (\texttt{Fixed} or \texttt{Dynamic}) together with \texttt{minOperators}, and the blueprint constrains which models are permitted via its \texttt{supportedMemberships} allow-list. There are no \texttt{queryMinOperators}, \texttt{queryMaxOperators}, or \texttt{queryExitDelay} hooks; operator exit timing is governed by the protocol's staking unbonding schedule (see Staking), not by a blueprint callback.

Within these models, custom \texttt{onRegister}/\texttt{onRequest} logic can still implement reputation-based entry, stake-weighted selection, or geographic distribution requirements.

\subsection{Job Configuration}

\textbf{requiresAggregation} determines whether results need multi-operator consensus.

\textbf{getAggregationThreshold(serviceId, jobIndex)} returns \texttt{(uint16 thresholdBps, uint8 mode)} per job: \texttt{thresholdBps} is the required agreement in basis points and \texttt{mode} selects how it is counted (\texttt{0} = count-based, \texttt{1} = stake-weighted). The protocol default is \texttt{6700}/count-based (two-thirds by operator count).

\subsection{Hook Versioning (MBSM)}

The protocol routes hook calls into a blueprint's service manager through the Master Blueprint Service Manager (MBSM). The \texttt{MBSMRegistry} tracks MBSM versions, and each blueprint is pinned to the MBSM version it was created against. The set and ABI of lifecycle hooks (\texttt{onRegister}, \texttt{onRequest}, \texttt{onApprove}, \texttt{onUnappliedSlash}, \texttt{onServiceTermination}, and the query hooks) are therefore \emph{version-bound}: a given MBSM version defines exactly which hooks exist and their signatures. When adopting a newer MBSM version, recheck the hook ABIs against that version rather than assuming forward compatibility.
...
\texttt{TangleProducer} watches blockchain events via standard EVM JSON-RPC:
\begin{itemize}[noitemsep]
    \item Queries Tangle contract logs
    \item Filters for the configured service ID
    \item Extracts job parameters and metadata
    \item Retries transient RPC failures with backoff
\end{itemize}
...
When your service needs to react to external systems (monitoring, exchanges, schedulers), operators can expose HTTP
endpoints that translate inbound requests into job executions. This is implemented as a gateway (HTTP server) plus a
producer that injects \texttt{JobCall}s into the runner.

\subsection{x402 Paid Triggers}

For HTTP-first services that want stablecoin settlement on EVM chains, operators can optionally accept paid HTTP job
calls via the x402 protocol. After payment is verified and settled, the request is translated into a job execution.
...
\begin{itemize}[noitemsep]
    \item \texttt{EpochKeeper} for epoch-boundary reward distribution (treasury-funded emission paid out from the pre-funded InflationPool)
...
and the full cap is minted at genesis across four buckets (Substrate, EVM, Treasury, Foundation), so \texttt{totalSupply} equals the cap from block zero. The InflationPool cannot mint; it only distributes TNT it already holds. ``Inflation'' here is treasury-funded \emph{emission}: roughly 1\% of supply per year ($\approx$ 1{,}000{,}000 TNT)
is transferred from the Treasury bucket into the InflationPool and paid out at epoch boundaries. This dilutes no one and creates no new supply. A governance \texttt{MINTER\_ROLE} exists and can mint only up to the cap, but with genesis already at the cap it has no headroom (a \texttt{mint()} that would exceed the cap reverts).
...
\begin{itemize}[noitemsep]
    ...
    \item \texttt{getAggregationThreshold(serviceId, jobIndex)} returns \texttt{(uint16 thresholdBps, uint8 mode)} (default \texttt{6700}/count-based)
\end{itemize}
...
Operators should assume transient RPC failures during result submission (rate limits, timeouts, nonce conflicts, temporary insufficient funds). The SDK surfaces submission errors to the runner. In practice, operators run blueprints under a supervisor (systemd, Kubernetes) and apply retries at the process or RPC layer.
...
EVM chains can reorganize. If your service requires stricter finality, consider polling behind the head block, or reprocessing a small block window on restart. Job handlers should be idempotent where possible.

\subsection{Crash Recovery}

The simplest recovery strategy is to restart the process and resume from a known block height. The producer supports starting from a specific block; operators can persist the last processed block (or a conservative checkpoint) in their own storage.

\begin{lstlisting}
let client = env.tangle_client().await?;
let service_id = env.protocol_settings.tangle()?.service_id.unwrap();

// Resume from a stored checkpoint (or start from genesis)
let start_block: u64 = load_checkpoint().unwrap_or(0);

BlueprintRunner::builder(TangleConfig::default(), env)
    .router(router())
    .producer(TangleProducer::from_block(client.clone(), service_id, start_block))
    .consumer(TangleConsumer::new(client))
    .run()
    .await
\end{lstlisting}
...
On shutdown, operators typically rely on their process supervisor and implement application-level draining when needed. Job handlers should be cancellation-safe and side-effect aware.
...
QoS provides data; slashing provides consequences. Connecting the two turns service-level objectives into enforceable penalties.

\textbf{Architecture}: QoS metrics flow from operators to a monitoring service. When metrics violate thresholds, the monitor proposes a slash via Tangle. The service manager's \texttt{querySlashingOrigin} hook authorizes the monitor. After the dispute window, slashing executes.

The on-chain entry point is \texttt{proposeSlash(uint64 serviceId, address operator, uint16 slashBps, bytes32 evidence)}. The amount is in \emph{basis points}, the call \emph{reverts on zero evidence}, the effective slash is capped at the service's \texttt{maxSlashBps} (\texttt{5000} $=$ 50\% at mainnet launch), and the realized penalty is scaled by the operator's committed exposure to the service. The examples below pass a non-zero \texttt{evidence} hash committing to the off-chain measurement.
    ...
    function checkAndSlash(uint64 serviceId, address operator) external {
        ...
        if (missed > MAX_MISSED) {
            bytes32 evidence = keccak256(
                abi.encode(serviceId, operator, missed, block.timestamp)
            );
            // slashBps in basis points (500 = 5%); zero evidence reverts
            tangle.proposeSlash(serviceId, operator, 500, evidence);
        }
    }
...
contract LatencySlashingManager {
    ...
    function reportLatencyViolation(
        ...
        if (violations[serviceId][operator] >= VIOLATION_THRESHOLD) {
            bytes32 evidence = keccak256(
                abi.encode(serviceId, operator, actualLatencyMs, proof)
            );
            // slashBps in basis points (1000 = 10%); zero evidence reverts
            tangle.proposeSlash(serviceId, operator, 1000, evidence);
        ...
        if (block.timestamp > r.windowStart + WINDOW) {
            if (r.totalChecks > 0) {
                ...
                if (pct < MIN_UPTIME_PERCENT) {
                    // Liveness penalty is a small band: 10 bps per missing
                    // percentage point, capped at 200 bps (2%). Liveness
                    // faults are not equivocation; large slashes are reserved
                    // for safety violations. Any value above the service's
                    // maxSlashBps is further capped by the protocol on execute.
                    uint256 slashBps = (MIN_UPTIME_PERCENT - pct) * 10;
                    if (slashBps > 200) slashBps = 200;
                    bytes32 evidence = keccak256(
                        abi.encode(serviceId, operator, pct, r.windowStart)
                    );
                    tangle.proposeSlash(
                        serviceId, operator, uint16(slashBps), evidence
                    );
                }
            }
...
\textbf{Dispute-game guardrails}: at mainnet launch a slash proposal opens a dispute that requires a \texttt{disputeBond} of 0.02 ETH (refunded if the dispute is cancelled, forfeited if the slash executes). The \texttt{disputeResolutionDeadline} is 21 days and is \emph{fail-open}: if the dispute is not resolved in time the slash auto-executes. Because resolution is fail-open, blueprints relying on QoS slashing should run an $m$-of-$n$ set of independent monitors so a single offline monitor cannot block a justified slash, and conversely so a single faulty monitor cannot force one. An operator with any pending slash is frozen from unstaking, which prevents stake-exit during the dispute window.
...
This section uses a few economic terms defined fully in the Tangle economics paper, summarized here: \emph{PfC} (profit-from-corruption) is the most an attacker can extract by violating the service; \emph{CoC} (cost-of-corruption) is the stake an attacker must put at risk to do so; \emph{exposure} is the fraction of an operator's stake committed to a given service and thus slashable for it; and the \emph{security ratio} is CoC$/$PfC. These are nominal upper bounds; the realized figures depend on how reliably violations are detected and slashed.
...
\textbf{Slashing realization.} The slash amount $S$ in these bounds is \emph{nominal}. The amount actually lost by a corrupt operator is closer to $P_{\text{execute}} \times P_d \times S_{\text{nominal}}$, where $P_{\text{execute}}$ accounts for the dispute game: at launch the dispute window is fail-open over 21 days, but a sophisticated attacker may still attempt to exit stake before execution (mitigated, not eliminated, by the pending-slash unstake freeze). Treat the headline CoC and security ratio as conditional bounds that hold only under the stated detection assumptions (probe-indistinguishability: challenges are statistically indistinguishable from real jobs) and collusion assumptions (no coalition above the corruption threshold).

\textbf{Deterrence versus the verifier's private incentive.} The condition $P_d \times S > V$ above is a \emph{social} deterrence condition: it tells you whether cheating is unprofitable in aggregate. It is distinct from whether any individual party is privately paid enough to \emph{run} the verification --- the verifier's dilemma. Detection has a real cost $C_v$, yet slash proceeds are routed by the protocol fee split rather than handed to whoever reported the violation. To close this gap, blueprints should fund an explicit reporter bounty or lean on the protocol's permissionless keeper rebate (see Payment Models) so that producing and submitting evidence is itself EV-positive.
...
A sandbox service promising agents run on a specified frontier model (the configured model, e.g. a named Claude, GPT, or Llama variant) with specified isolation.

\textbf{Verification mechanisms}:
\begin{itemize}[noitemsep]
    \item Model fingerprinting: Challenge database of (prompt, expected pattern) pairs distinguishing the promised model from alternatives
...
\textbf{Slashing configuration} (each subject to the per-service \texttt{maxSlashBps} ceiling, 50\% at mainnet launch):
\begin{itemize}[noitemsep]
    \item Model substitution: 50\% (severe, violates core promise)
    \item Performance degradation: 5\% (moderate, affects quality)
    \item Isolation breach: configured at 100\%, but realized slash is capped at \texttt{maxSlashBps} (50\% at launch) unless governance raises the per-service ceiling for an equivocation-class fault
\end{itemize}

\textbf{Quantified economics.} Customer runs trading agents managing \$200,000. PfC = \$200,000 (assets under management).

Service uses 3 operators, each staking \$100,000 with 80\% exposure. Corrupting requires all 3 (any honest operator detects substitution via challenges, under the probe-indistinguishability and no-collusion-above-threshold assumptions). The \emph{nominal} stake at risk is $3 \times \$100,000 \times 0.8 = \$240,000$. With the launch \texttt{maxSlashBps} of 50\%, however, a single corruption event can burn at most 50\% of each committed stake, so the \emph{realizable} CoC for a one-shot attack is $3 \times \$100,000 \times 0.8 \times 0.5 = \$120,000$.

Security ratio against realizable CoC = \$120,000 / \$200,000 = 0.6, below 1 --- the 50\% cap means a single substitution event is not deterred by stake alone at these parameters. Restoring a ratio above 1 (the economics paper's target band; the previously cited ``1.5x'' is illustrative, not a protocol constant) requires raising stake or operator count, reducing exposure to a smaller AUM cap per service, or governance-raising \texttt{maxSlashBps} for the equivocation-class substitution fault.

\textbf{Detection probability.} Model fingerprinting challenges submitted 10 times daily. Each detects substitution with 85\% probability, \emph{assuming probes are indistinguishable from real jobs}; an operator who can identify probes can pass them while cheating real traffic, collapsing $P_d$. Over a 7-day window:
\[
P_d = 1 - (1 - 0.85)^{70} \approx 99.99\%
\]

Under that assumption detection is near-certain, but the deterrent is still bounded by the realizable slash above: effective expected cost to the attacker is approximately $P_{\text{execute}} \times P_d \times \$120,000$, not the nominal \$240,000.
...
\textbf{Slashing configuration} (realized slash capped at the per-service \texttt{maxSlashBps}, 50\% at launch):
\begin{itemize}[noitemsep]
    \item Source forgery: configured at 100\% (safety/equivocation class), realized at the \texttt{maxSlashBps} ceiling unless governance raises it for this service
    \item Price deviation: 10\% per incident, accumulating up to the ceiling
    \item Availability failure: a small liveness band of $\sim$10 bps (0.1\%) per missed update, capped at 200 bps (2\%) --- liveness faults are not equivocation and should not approach the safety ceiling
\end{itemize}

\textbf{Economics.} Oracle serves DeFi with \$100M exposure. Manipulated prices enable \$10M arbitrage (PfC = \$10M).

5 operators each staking \$3M with 100\% exposure. The nominal stake of a 3-of-5 coalition is \$9M, but with the 50\% launch \texttt{maxSlashBps} the realizable one-shot CoC is $\$9M \times 0.5 = \$4.5M$.

Security ratio against realizable CoC = 0.45 --- insufficient under the no-collusion-above-threshold assumption. The blueprint should increase stake requirements, raise the corruption threshold, reduce exposure to smaller protocols, or have governance raise \texttt{maxSlashBps} for the source-forgery (equivocation) fault.
...
\textbf{Property V3 (Economic Sufficiency)}: For all violation types $M$ with benefit $V_M$: $p_M \times S_M > V_M$, where $S_M$ is the \emph{realizable} slash, i.e. $S_M \le \texttt{maxSlashBps} \times (\text{committed exposure})$.

\textit{Importance}: Core security condition. Blueprints must configure detection and slashing to satisfy this. Note the hard ceiling: if $V_M$ exceeds $\texttt{maxSlashBps}$ times an operator's committed stake (50\% at launch), no detection probability or slash schedule deters that operator alone --- the only remedies are raising stake, increasing the number of operators required to corrupt, reducing per-service exposure, or governance-raising \texttt{maxSlashBps} for that equivocation-class fault.
...
Blueprints interact with the Tangle marketplace for service discovery and pricing. Developers need this flow to design services that can price, launch, and settle correctly.

\subsection{Payment Models}

A service selects one of four payment models at request time:

\begin{description}[noitemsep]
    \item[PayOnce] a single up-front payment for the service term.
    \item[Subscription] recurring billing. Billing is permissionless and TWAP-fair: anyone may call \texttt{billSubscription} when a period is due, and the call carries a 0.5\% keeper rebate (carved from the protocol's share) so that running the keeper is EV-positive.
    \item[EventDriven] pay-per-job, settled as jobs are executed.
    \item[RFQ quotes] operator-signed, EIP-712 typed quotes accepted on-chain (see The RFQ System).
\end{description}

\textbf{Fee split.} Each service-fee payment is split across five recipients in basis points summing to 100\%: operator 4000 (40\%), developer 2000 (20\%), staker 2000 (20\%), protocol 1950 (19.5\%), and keeper 50 (0.5\%). The keeper's 0.5\% is the rebate carved out of the protocol share that makes permissionless subscription billing worthwhile. The developer's 2000-bps share is also the source from which \texttt{BuybackBlueprintBase} funds token buybacks.
...
\begin{enumerate}[noitemsep]
    ...
    \item Signs the quote as EIP-712 typed data over the whole \texttt{QuoteDetails} struct
    \item Returns the signed quote; spam resistance comes from the expiry timestamp and the operator's signature, with acceptance enforced on-chain (there is no proof-of-work)
\end{enumerate}

The signed \texttt{QuoteDetails} struct contains: blueprint ID, TTL in \emph{seconds} (enforced against \texttt{block.timestamp}; the struct field is named \texttt{ttlBlocks} for legacy reasons but the unit is seconds), total cost, timestamp, expiry, and security commitments (which assets back the operator's stake exposure). Because the EIP-712 signature covers the entire struct, any tampering with these fields invalidates the quote, and acceptance is verified on-chain at service creation.
...
Prices adjust as operators observe market conditions. On-chain service creation reveals accepted prices. Operators analyze historical prices and update quotes.
...
The \texttt{agg-sig-gossip} feature provides the specialized gossip transport for the BLS signature aggregation flow described in the \emph{BLS Aggregation} section: operators sign their results with BLS keys, broadcast the signatures over the per-job gossip topic, and an aggregator combines them once the threshold is met before submitting the aggregated result on-chain.
...
\begin{description}
    \item[http\_rpc\_url] EVM JSON-RPC endpoint for the host chain (local default: \texttt{http://127.0.0.1:8545}, the Anvil port; Base mainnet uses chainId \texttt{8453})
    \item[ws\_rpc\_url] WebSocket EVM JSON-RPC endpoint for event subscriptions (local default: \texttt{ws://127.0.0.1:8545})
    ...
    \item[protocol] Target protocol: \texttt{tangle} (the only protocol supported at launch; \texttt{eigenlayer} and \texttt{symbiotic} are planned)
...
\begin{itemize}[noitemsep]
    \item \textbf{Blueprint SDK Repository}: \url{https://github.com/tangle-network/blueprint}
