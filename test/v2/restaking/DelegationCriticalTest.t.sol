...
contract DelegationCriticalTest is DelegationTestHarness {
    ...
    // ═══════════════════════════════════════════════════════════════════════════════
    // SECTION 9: REWARDS + UNSTAKE INTERACTION TESTS (Medium)
    // ═══════════════════════════════════════════════════════════════════════════════

    /// @notice Test rewards accrue correctly before unstake
    function test_RewardsAccrueBeforeUnstake() public {
        _depositAndDelegate(delegator1, operator1, 10 ether);

        // Add rewards
        _addRewards(operator1, 10 ether);

        // Check pending rewards (delegator gets 90% = 9 ether due to 10% commission)
        uint256 pending = delegation.getPendingDelegatorRewards(delegator1);
        assertGt(pending, 0, "Should have pending rewards");
    }

    /// @notice Test rewards must be claimed before full unstake (rewards tied to shares)
    ...
    function test_CanClaimRewardsAfterFullUnstake() public {
        _depositAndDelegate(delegator1, operator1, 10 ether);
        _addRewards(operator1, 10 ether);
        ...
        assertGt(claimed, 0, "Should have claimed rewards before unstake");
        ...
        // After full unstake, no more rewards should accrue
        assertDelegationEq(delegator1, operator1, 0);
    }

    /// @notice Test partial unstake doesn't lose rewards
    function test_PartialUnstakePreservesRewards() public {
        ...
        // Claim - should get at least the rewards from before
        uint256 balanceBefore = delegator1.balance;
        vm.prank(delegator1);
        delegation.claimDelegatorRewards();

        assertGt(delegator1.balance - balanceBefore, 0, "Should have received some rewards");
    }

    /// @notice Test operator commission works correctly
    function test_OperatorCommissionCorrect() public {
        _depositAndDelegate(delegator1, operator1, 10 ether);

        // Add 10 ETH rewards - 10% commission = 1 ETH to operator, 9 ETH to delegators
        _addRewards(operator1, 10 ether);

        uint256 operatorRewards = delegation.getPendingOperatorRewards(operator1);
        assertEq(operatorRewards, 1 ether, "Operator should have 1 ETH commission");

        uint256 delegatorRewards = delegation.getPendingDelegatorRewards(delegator1);
        assertEq(delegatorRewards, 9 ether, "Delegator should have 9 ETH rewards");
    }
