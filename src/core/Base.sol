    ...
    TangleStorage
{
    ...
    ///      Lives in `Base` so both the payment-billing facet (bill-time scale recovery)
    ///      and the service-lifecycle facet (join-time identity seeding) share ONE
    ///      definition of the witness — a duplicated loop could drift and corrupt it.
