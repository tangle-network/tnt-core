...
contract GlacisMessageReceiver is GlacisAccessControlClient, IGlacisClient {
    error GlacisClient__CanOnlyBeCalledByRouter();
    error GlacisClient__InvalidRouterAddress();

    event GlacisClient__MessageRouted(bytes32 indexed messageId, uint256 toChainId, bytes32 to);

    event GlacisClient__MessageArrived(address[] fromAdapters, uint256 fromChainId, bytes32 fromAddress);

    address public immutable GLACIS_ROUTER;

    constructor(address _glacisRouter, uint256 _quorum) GlacisAccessControlClient() IGlacisClient(_quorum) {
