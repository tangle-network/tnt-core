...
interface IAssets {
    /// @notice Get the next available asset ID
    /// @return id The next available asset ID that can be used to create a new asset
    function next_asset_id() external view returns (uint256 id);
    ...
    function create(uint256 id, address admin, uint256 minBalance) external returns (bool success);

    /// @notice Start the process of destroying an asset
    /// @param id The identifier of the asset to destroy
    /// @return success True if the operation was successful
    function startDestroy(uint256 id) external returns (bool success);
    ...
    /// @return success True if the operation was successful
    function mint(uint256 id, address beneficiary, uint256 amount) external returns (bool success);
    ...
    /// @return success True if the operation was successful
    function transfer(uint256 id, address target, uint256 amount) external returns (bool success);
