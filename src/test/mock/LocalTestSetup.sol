...
import { GlacisMockSetup, GlacisCommons, GlacisRouter, GlacisLayerZeroV2Adapter, GlacisHyperlaneAdapter } from "glacis-test/GlacisMockSetup.sol";
...
contract LocalTestSetup {
    ...
    function setup() internal returns (Config memory config) {
        ...
        config.xcConfig.remoteChainBridgeManagerTangle = setupRemoteChainBridgeManager(
            address(config.glacisConfig.glacisRouter),
            0,
            bytes32(uint256(uint160(XC_EVM_SIDE))),
            address(this),
            block.chainid
        );
        config.xcConfig.remoteChainBridgeManagerXC = setupRemoteChainBridgeManager(
            address(config.glacisConfig.glacisRouter),
            0,
            bytes32(uint256(uint160(TANGLE_SIDE))),
            address(this),
            block.chainid
        );
        config.xcConfig.masterVault = setupMasterVault(address(config.xcConfig.remoteChainBridgeManagerTangle), address(config.glacisConfig.glacisRouter), 1);
    ...
    function createFees(
        GlacisCommons.CrossChainGas[] storage fees,
        uint256 amount,
        uint256 size
    ) internal {
        for (uint256 i; i < size; ++i) {
            fees.push(GlacisCommons.CrossChainGas({ 
                gasLimit: 0,
    ...
    function setupRemoteChainBridgeManager(address _glacisRouter, uint256 _quorum, bytes32 _receiver, address _refundAddress, uint256 _tangleChainId) internal returns (RemoteChainBridgeManager) {
