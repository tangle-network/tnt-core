...
/// @notice Facet for delegation lifecycle (delegate in / schedule / undelegate / blueprint edits).
/// @dev The single-step "redeem" path (executeDelegatorUnstakeAndWithdraw) lives in
///      StakingUnstakeWithdrawFacet — split out because the combined facet sat at the EIP-170
///      runtime-size ceiling and its CREATE exceeded the per-tx gas cap on chains that meter code
///      deposit aggressively (Tempo caps a tx at 30M gas; the combined facet deployed at ~33M).
