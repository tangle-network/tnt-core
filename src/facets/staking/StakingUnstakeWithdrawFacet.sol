...
/// @title StakingUnstakeWithdrawFacet
/// @notice Facet for the single-step "redeem" path — execute a matured bond-less unstake and
///         withdraw the resulting assets in one transaction.
/// @dev Split out of StakingDelegationsFacet: that facet sat at the EIP-170 runtime-size ceiling
///      AND its CREATE exceeded the per-transaction gas cap on chains that meter code deposit more
///      aggressively (e.g. Tempo caps a tx at 30M gas; the combined facet's deploy cost ~33M).
///      Both facets extend the same StakingFacetBase, so they share storage and behavior verbatim;
///      only the `executeDelegatorUnstakeAndWithdraw` selector routes here instead.
contract StakingUnstakeWithdrawFacet is StakingFacetBase, IFacetSelectors {
    ...
    function selectors() external pure returns (bytes4[] memory selectorList) {
        ...
        selectorList[0] = this.executeDelegatorUnstakeAndWithdraw.selector;
    }
