...
```bash
forge soldeer install tnt-core~0.10.7
```
...
Extend `BlueprintServiceManagerBase` in Solidity and implement the `onRequest` and `onJobResult` hooks. Install TNT Core via `forge soldeer install tnt-core~0.10.7` and import the base contract. See the Quick Start section above for a working example.
