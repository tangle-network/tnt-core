...
- `src/lockups/TNTLockFactory.sol` + `src/lockups/TNTCliffLock.sol`: per-beneficiary cliff locks.
...
Claims split into:
- `unlockedBps = 1000` (10%) transferred immediately to the recipient.
- `lockedAmount` (90%) transferred to a deterministic `TNTCliffLock` for the recipient.
- `unlockTimestamp = deploymentTime + 180 days` by default.

You can update lock config via `TangleMigration.setLockConfig(...)` only while `totalClaimed == 0` (before the first claim).

Locked tokens are created with `delegatee = recipient` so (if the token supports `IVotes`) the recipient can use voting power while tokens are locked.
